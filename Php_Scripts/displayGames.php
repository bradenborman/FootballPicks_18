<?php
	...
	/*
		NEED TO CHANGE USERNAME FROM HARDCODED TO APPRO LOGIN
	*/
	...
	$sql = "SELECT * FROM `Games` LEFT JOIN Pick_History ON Pick_History.Game_ID = Games.Game_ID WHERE Week_ID = $weekSelected";	
	...
	$GameCounter = 1;
		...
		while ($row = mysqli_fetch_array($AllGames))
		{
			...
			echo '<div class="row secondaryInfo">';
				...
				if ($isUpset) echo 'Upset Eligible'; 
				echo '</div>';
			echo '</div><hr class="divider">';
			echo '<div class="row teamRow">';
				echo '<div class="col-xs-2 teamcol text-right">';
				echo ''.$Team1_Picked_Percent.'%';														
				echo '</div>';
			if($didPickedTeamOne)
				echo '<div class="col-xs-9 team '.$Team1.' game'.$GameCounter.' picked">';
			else  	echo '<div class="col-xs-9 team '.$Team1.' game'.$GameCounter.' unpicked">';
			...
			echo '<div class="row teamRow">';
				...
				echo '</div>';
			if($didPickedTeamTwo) {
					 echo '<div class="col-xs-9 team '.$Team2.' game'.$GameCounter.' picked">'; 				
			}
			else { 			
					 echo '<div class="col-xs-9 team '.$Team2.' game'.$GameCounter.' unpicked">'; 
			}	
